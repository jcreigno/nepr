(function ($, ko) {
  ...
  function AppErrorsViewModel() {
    ...
    self.maxPageIndex = ko.computed(function () {
      return Math.ceil(self.all().length / self.pageSize()) - 1;
    });
  ...
  function AppViewModel() {
    ...
    self.loadStats = function () {
      $.getJSON(apiUrl('/stats/' + self.environnement().code, self.dateRange()), function (data) {
        ...
        sortedData.forEach(function (item) {
          ...
          var group = groupedStats[couche] || (groupedStats[couche] = []);
          group.push(item);
        });
    ...
    self.graphPerfs = function () {
      var todate = function (str) {
        var d = str.substring(0, str.indexOf(',')).split(' ').join('T');
        return new Date(d);
      };
      ...
      // chart data
      var chartData = data.slice(data.length - 100, data.length).reverse().map(function (item) {
      ...
      var divs = d3.select("#perfChart").selectAll("div").data(chartData);
      ...
      divs.style("height", function (d) {
        ...
        $.sammy().setLocation(self.urlFor('traces', {
  ...
  $(document).ready(function () {
    ...
    $.sammy(function (app) {
      ...
      this.get('#/:env/:page', function (context) {
        ...
        model.environnement(model.allEnvironnements().filter(function (item) {
